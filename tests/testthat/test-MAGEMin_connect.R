...
# in MAGEMin algorithm or a recalibration of the DB.
  ...
  expect_equal(
    getSSComp(Minimize_ATAC4(),"pl","emmol"),
...
test_that("Arguments are properly passed to MAGEMin",{
  ...
  expect_match(
    paste0(names(getPhProp(Minimize_ATAC4())),collapse=" "),
    "pl"
  )

  expect_match(
    paste0(names(getPhProp(Minimize_ATAC4(nameSolvus=F))),collapse=" "),
    "fsp"
  )
